...
public class DeleteCouponView {

	private JFrame frame;
	private JTable table;

	public DeleteCouponView(String couponCode, String memberName, String memberCode, int prepayMoney,
			DefaultTableModel tableModel, CapnhatController cnc) {
		initialize(couponCode, memberName, memberCode, prepayMoney, tableModel, cnc);
	}
	...
	private void initialize(final String couponCode, final String memberName, final String memberCode,
			final int prepayMoney, final DefaultTableModel tableModel, final CapnhatController cnc) {
		frame = new JFrame();
		...
		JButton btnIn = new JButton("  Xoá  ");
		btnIn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				PhieuMuonDAO pmd = null;
				pmd.delete(couponCode);
				cnc.loadCoupon();
